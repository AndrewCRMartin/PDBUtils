...
extern void ContourText(char *s, REAL x, REAL y);
extern void Polyline(int n, LIST *list);

/* Contour generation */
void Contour(REAL *data,
             int dim_x,
             int dim_y,
             double inc);
...
static UCHAR savePoint(GRID *grid,
                       POINT *point,
                       REAL level);
static REAL getDataPoint(GRID *grid,
                          POINT *point,
                          UCHAR corner);

/************************************************************************/
/*>void Contour(REAL *data, int dim_x, int dim_y, double inc)
   -----------------------------------------------------------
...
static void startLine(GRID *grid)
{
       ...
       level+=grid->step, idx++)
   {
      ...
      for(edge=0; edge < 4; edge++)
         startEdge(grid, (REAL)level, edge);
      /* Check interior points */
      startInterior(grid, (REAL)level);
   }
}

/************************************************************************/
static void startEdge(GRID *grid, REAL level, UCHAR bearing)
{
   POINT point1,
         point2;
   REAL last,
...
/************************************************************************/
/*>void startInterior(GRID *grid, REAL level)
   -------------------------------------------
...
/************************************************************************/
/*>void drawLine(GRID *grid, POINT *point, REAL level)
   ----------------------------------------------------
...
/************************************************************************/
/*>savePoint(GRID *grid, POINT *point, REAL level)
   ------------------------------------------------
   Add specified point to the contour point list
   13.07.92 Typed
*/
static UCHAR savePoint(GRID *grid, POINT *point, REAL level)
{
   REAL last, next;
   REAL x,y,ds;
   char s[80];
   ...
   grid->list[grid->count].x = x / (REAL)(grid->dim_x - 1);
   grid->list[grid->count].y = y / (REAL)(grid->dim_y - 1);
...
*/
static REAL getDataPoint(GRID *grid, POINT *point, UCHAR corner)
{
   UCHAR dx, dy;
   USHORT offset;
